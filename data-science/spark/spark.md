...
Spark Structured Streaming is a stream processing engine based on Spark SQL. It
allows you to write SQL queries against a series of data with results updating
in real time. 

The key idea in Structured Streaming is to treat a live data stream like a table
that is being constantly appended to. You write your SQL queries against a
streaming source like you would for a batch table
...
Spark Streaming provides a high level "discretized stream" (DStream), which
...
* Batches are processed in Spark and sent to an output stream (write to file,
  db, etc..)
* You can execute Spark SQL or MLLib on streams.
* You can join multiple input streams.
* Streams can have windows, allowing you to perform calculations over time
  periods or multiple batches.

## Spark MLLib

Provides ML routines to Spark. Allows you to run ML algorithms using Spark's
scalable prgramming model.
